...
(def mem-resolver-spec (memoize resolver-spec))

(defn resolve-content
  ([root ent content-k content] (resolve-content root ent content-k content identity))
  ([{:as root :keys [->content-spec contents-hiccup-wrapper]} ent content-k content f]
   (let [[type] (u/conform! (mem-resolver-spec (->content-spec ent)) content)]
     (case type
       :content
