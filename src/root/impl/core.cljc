...
(defn normalize
  ...
  ([root data]
   (normalize {} root data))
  ([out {:as root :keys [->ref ->content-keys ->entity-spec]} data]
   (let [data-ref (->ref data)
         out      (assoc out data-ref data)]
     (if-let [content-ks (and data (not-empty (->content-keys data)))]
       (reduce
        (fn [out content-k]
          (if-let [content-val (get data content-k)]
            (let [[type conformed]
                  (u/conform! (rr/mem-resolver-spec (->entity-spec data)) content-val)]
              (case type
                :content
                ...
                :contents
                (let [data-ref     (->ref data)
                      content-refs (mapv ->ref content-val)]
                  (as-> out out
                        (assoc-in out [data-ref content-k] content-refs)
                        (reduce (fn [out d] (normalize out root d)) out content-val)))

                :content-map
                (reduce-kv
                 (fn [out k v]
                   (let [[inner-type] (get conformed k)]
                     (case inner-type
                       :entity
                       (-> out
                           (assoc-in [data-ref content-k k] (->ref v))
                           (normalize root v))

                       :entities
                       (as-> out out
                             (assoc-in out [data-ref content-k k] (mapv ->ref v))
